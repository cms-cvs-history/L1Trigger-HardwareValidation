...
if [ -e $ecalFile ]; then cp $ecalFile $targDir/.; fi;
if [ -e $hcalFile ]; then cp $hcalFile $targDir/.; fi;
if [ -e $rctFile ]; then cp $rctFile $targDir/.; fi;
if [ -e $gctFile ]; then cp $gctFile $targDir/.; fi;
scp "cms-bris-pc01.cern.ch:\"$scFile\"" $targDir/.

#copy config files
conf=`awk "/$pattId/ { print "'$4'" }" pattIndex.dat`
confFile=src/L1Trigger/HardwareValidation/test/$conf.cfg
